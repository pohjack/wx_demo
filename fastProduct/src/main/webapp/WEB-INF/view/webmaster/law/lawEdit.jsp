...
<head>
<meta charset="UTF-8">
<title>潮州烟草案件卷宗自动化系统-法律法规编辑</title>
							...
							<div class="tabbable-line boxless tabbable-reversed">						
										<div class="portlet box green-turquoise">
											<div class="portlet-title">
												<div class="caption">
													<i class="fa fa-plus-square"></i><c:if test="${not empty obj.id}" >编辑</c:if><c:if test="${empty obj.id}" >新建</c:if>法律法规信息
												</div>
											</div>
											<div class="portlet-body form">
												<div class="form-body">
													<div class="row">
														<div class="col-md-6">
															<div class="form-group">
																...
																<div class="col-md-8">
																	<input type="text" name="caseCauseNo" id="caseCauseNo"  <c:if test="${ not empty obj.id}">readonly</c:if>  <c:if test="${ empty obj.id}">onblur ="checkLawNum();"</c:if>  value="${obj.caseCauseNo }" maxlength="20" class="form-control">
																</div>
															</div>
														</div>
													</div>
													<div class="row">
														<div class="col-md-6">
															<div class="form-group">
																...
																<div class="col-md-8">
																	<textarea class="form-control" name="caseCauseCont" id="caseCauseCont" rows="3" maxlength="100">${obj.caseCauseCont }</textarea>
																</div>
															</div>
														</div>
													</div>
													<div class="row">
														<div class="col-md-6">
															<div class="form-group">
																<label class="control-label col-md-4">违反法规内容</label>
																<div class="col-md-8">
																	<textarea class="form-control" name="breakLaw" id="breakLaw" rows="3" maxlength="100">${obj.breakLaw }</textarea>
																</div>
															</div>
														</div>
													</div>
													<div class="row">
														<div class="col-md-6">
															<div class="form-group">
																...
																<div class="col-md-8">
																	<textarea class="form-control" name="punishLaw" id="punishLaw" rows="3" maxlength="100">${obj.punishLaw }</textarea>
																</div>
															</div>
														</div>
													</div>
													<div class="row">
														<div class="col-md-6">
															<div class="form-group">
																<label class="control-label col-md-4">鉴定结果表述内容</label>
																<div class="col-md-8">
																	<textarea class="form-control" name="authenRes" id="authenRes" rows="3" maxlength="100">${obj.authenRes }</textarea>
																</div>
															</div>
														</div>
													</div>
...
<script src="${pageContext.request.contextPath}/statics/script/forbitBackSpace.js" type="text/javascript"></script>
	...
	<script type="text/javascript">
		function submitClickOut(){
			var  result  =  checkLawNum();
			if(!result){
				return false ;
			}
			/* if($("#caseCauseNo").val() == ""){
				layer.tips('[案由编号]不能为空', '#caseCauseNo', {
				  tips: [1, '#3595CC'],
				  time: 4000
				});
				return false;
			} */
			if($("#caseCauseCont").val() == ""){
				layer.tips('[案由内容]不能为空', '#caseCauseCont', {
				  tips: [1, '#3595CC'],
				  time: 4000
				});
				return false;
			...
			if($("#breakLaw").val() == ""){
				layer.tips('[违反法规内容]不能为空', '#breakLaw', {
				  tips: [1, '#3595CC'],
				  time: 4000
				});
				return false;
			...
			if($("#punishLaw").val() == ""){
				layer.tips('[处罚法规]不能为空', '#punishLaw', {
				  tips: [1, '#3595CC'],
				  time: 4000
				});
				return false;
			...
			if($("#authenRes").val() == ""){
				layer.tips('[鉴定结果表述内容]不能为空', '#authenRes', {
				  tips: [1, '#3595CC'],
				  time: 4000
				});
				return false;
		...
		$("#saveBtn").click(function() {
			...
			if(submitClick){
				$.ajax({
					cache : true,
					type : "POST",
					url : "${pageContext.request.contextPath}/webmaster/law/doSave",
					...
					success : function(data) {
						...
						window.location.href="${pageContext.request.contextPath}/webmaster/law/toList";
					}
				});
			}
		});
	...
	function checkLawNum(){
		var flag= true;
		if($("#id").val()==""){
					var lawNum = $("#caseCauseNo").val();
			if(lawNum==""){
							layer.tips('[案由编号]不能为空', '#caseCauseNo', {
					 					 tips: [1, '#3595CC'],
					 					 time: 4000
				...
				$.ajax({
					cache : true,
					type : "POST",
					url : "${pageContext.request.contextPath}/webmaster/law/checkLawNum",
					data : {"lawNum":lawNum},// 你的formid
					async : false,
					dataType : "JSON",
					error : function(request) {
						layer.msg("Connection error");
					...
					success : function(data) {
						if(data.msg=="false"){
							flag=false;
							layer.tips('[案由编号]已经存在', '#caseCauseNo', {
								  tips: [1, '#3595CC'],
								  time: 4000
								});
						}
